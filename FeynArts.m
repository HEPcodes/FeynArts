...
This is FeynArts, Version 3
Copyright by Sepp Kueblbeck, Hagen Eck, and Thomas Hahn 1991-2001
last modified 19 Oct 01 by Thomas Hahn
...
Print["last revised 19 Oct 01"]


BeginPackage["FeynArts`"]

FullFileName::usage =
"FullFileName[name, path] returns the name of the file with its full
path appended."
...
Propagator::usage =
"Propagator[v1, v2] is an (undirected) propagator joining the two vertices
v1 and v2.\n
...
l loops and counter-term order cto. The starting topologies are defined in
Topology.m."
...
CTOrder::usage =
"CTOrder is an option of CreateTopologies that specifies for which
order counter terms shall be generated."
...
CanonicalOrder::usage =
"CanonicalOrder[top] orders the topology top into a more or less canonical
order."
...
InitializeModel. The model information is taken from the file GEN.gen."
...
"InitializeModel[MOD] initializes the classes model for the model MOD and
the generic model given by the GenericModel option. The model information
is taken from the file MOD.mod. InitializeModel[] initializes only the
generic model."
...
IndexSum::usage =
"IndexSum[expr, {i, imax}] represents the unevaluated sum of expr over the
index i from 1 to imax. To execute the sum, substitute IndexSum by Sum."
...
FieldPoints::usage =
"FieldPoints[cto] returns a list of field points of counter-term order cto
in the current model, FieldPoints[Classes] returns a list of field points
of all orders in the current classes model, and FieldPoints[Generic]
returns a list of all generic field points."
...
needed only when 1PI vertex-function insertions are generated (i.e. for
topologies created with CreateVFTopologies) to weed out such vertex
function that violate the conservation of a quantum number, as determined
by ViolatesQ."

ViolatesQ::usage =
"ViolatesQ is a function defined in the model file that determines whether
a vertex function violates quantum-number conservation. It is called by
InsertFields for every 1PI vertex-function insertion (i.e. when inserting
topologies created with CreateVFTopologies). It receives as arguments the
quantum numbers of the involved fields (times -1 for antiparticles) and
must return True if the vertex violates the conservation of those quantum
numbers."
...
model where lev = Generic or Classes. If a match is found, the permutation
is returned which brings the vertex vert into the reference order."
...
Numbering::usage =
"Numbering is an option of Paint. It can take the values Full, Simple, or
None. Full generates numbering of the form T1 C8 N15 (= topology 1,
classes insertion 8, running number 15), Simple generates just a running
number, and None omits the numbering altogether."

Full::usage =
"Full is a possible choice for the Numbering option of Paint. It specifies
that the numbering of the diagrams be of the form T1 C8 N15 (= topology 1,
classes insertion 8, running number 15)."
...
"PropagatorGraphics[type, arrow, label][from, to, height, labelpos] is a
graphics object representing a propagator."
...
Render::usage =
"Render[g, format] renders the FeynArtsGraphics object g. The output is a
string for the \"PS\", \"EPS\", and \"TeX\" formats and a regular Graphics
object for all other formats."
...
$FeynArtsDir = DirectoryName[ FullFileName[$Input, $Path] ]


LoadPackage[ name_ ] :=
  Get[ ToFileName[{$FeynArtsDir, "FeynArts"}, name] ]

LoadPackage["Utilities.m"]

LoadPackage["Topology.m"]

LoadPackage["Initialize.m"]

LoadPackage["Insert.m"]

LoadPackage["Analytic.m"]

LoadPackage["Graphics.m"]
