...
WriteTeXFile[modelfile_, opt___Rule] :=
...
mod, FieldRules, class, couplings, hh},
  ...
  mod = ToString[modelfile];
  If[ StringPosition[mod, "."] === {}, mod = mod <> ".mod" ];
  Check[Get[mod], Abort[]];
  ...
  If[ texfile === Automatic,
    texfile = StringReplace[mod, ".mod" -> ".tex"] ];
